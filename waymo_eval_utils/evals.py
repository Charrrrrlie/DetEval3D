# PyTorch Dataloader and Evaluation Tools for Waymo Open Dataset
# Reference https://github.com/PJLab-ADG/3DAL
# Written by PJLAB-ADG
...
class WaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def get_waymo_filter(self, infos, class_name, distance_thresh=100, is_gt=False, fake_gt_infos=True):

        frameid, boxes3d, type, score, overlap_nlz, difficulty = self.generate_waymo_type_results(
            infos, class_name, is_gt=is_gt, fake_gt_infos=fake_gt_infos
        )

        if is_gt:
            boxes3d, frameid, type, score, difficulty = self.mask_by_distance(
                distance_thresh, boxes3d, frameid, type, score, difficulty
            )
            return frameid, boxes3d, type, score, difficulty

        else:
            boxes3d, frameid, type, score, overlap_nlz = self.mask_by_distance(
                distance_thresh, boxes3d, frameid, type, score, overlap_nlz
            )
            return frameid, boxes3d, type, score, overlap_nlz
